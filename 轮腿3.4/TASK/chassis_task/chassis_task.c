...
float fp_Limit_mouse(float x,float min,float max)
{
	if(x>=max) return max;
...
void chassis_L_init()//1200 30000   300
{
	...
	leg_left.roll_pid.Kp=3.0;	  leg_left.roll_pid.Ki=0.01;    leg_left.roll_pid.Kd=-2.0f;   leg_left.roll_pid.max_iout=20;    leg_left.roll_pid.max_out=60.0f;//40
...
//		}
	...
	/*****************left************************/	
		...
		leg_left.L0_set=0.20f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f-0.30f*arm_sin_f32((Angular_Handler.Pitch-pitch_offect)*TheattoRad);	
...
//			Can_M1505b_I(0,aaa);		
			...
			else if(fabs(Angular_Handler.ROLL-MPU_offect)>25.0f&&leg_left.step_flag==0)
			{
				leg_left.recover_flag=0;
			}			
...
//		leg_right.x_set=leg_right.x_set+leg_right.v_set*Chassis_task_Time;
		...
		leg_right.L0_set=0.20f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f+0.30f*arm_sin_f32((Angular_Handler.Pitch-pitch_offect)*TheattoRad);
...
//			Can_M1505b_I(0,aaa);
			...
			else if(fabs(Angular_Handler.ROLL-MPU_offect)>25.0f&&leg_right.step_flag==0)
			{
				leg_right.recover_flag=0;
			}
