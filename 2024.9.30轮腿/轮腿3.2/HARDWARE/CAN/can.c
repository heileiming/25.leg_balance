...
void Can_M1505b_ID(uint8_t ID)
{
		...
		TxMessage.StdId=0x108;																																												 					 // 标准标识符 				   																																								 // 设置扩展标示符 
		TxMessage.IDE=CAN_Id_Standard;						 																																		 					 // 标准帧
		TxMessage.RTR=CAN_RTR_Data;		 																																													 // 数据帧
		TxMessage.DLC=0x08;		
		TxMessage.Data[0] =	ID;
...
//		u8 mbox;
		...
		TxMessage.StdId=0x32;																																												 					 // 标准标识符 				   																																								 // 设置扩展标示符 
		TxMessage.IDE=CAN_Id_Standard;						 																																		 					 // 标准帧
		TxMessage.RTR=CAN_RTR_Data;		 																																													 // 数据帧
		TxMessage.DLC=0x08;		
		TxMessage.Data[0] =	current1>>8;
		TxMessage.Data[1]	=	current1;
		TxMessage.Data[6] =	current2>>8;
		TxMessage.Data[7]	=	current2;	
...
//	 while((CAN_TransmitStatus(CAN2, mbox)!=CAN_TxStatus_Ok));	//等待发送结束		
...
void CAN2_RX1_IRQHandler(void)
{
    ...
    CAN_Receive(CAN2, CAN_FIFO1, &RxMessage2);
		...
		switch (i)
		{
			case 0x97: M1505B_motor_measure(M1505B_data[0],RxMessage2); break;
			case 0x9A: M1505B_motor_measure(M1505B_data[1],RxMessage2); break;
