...
//	for(i=0;i<Len;i++)
//	{  
//		res[USB_USART_RX_STA]=Buf[i]; 
//		if(res[0] == 0x5a)			//接收刚开始
//		{
//			USB_USART_RX_STA++;
//			USB_USART_RX_STA=USB_USART_RX_STA%MINIPC_FRAME_LENGTH;
//		}
//		else
//		{
//			memset(res,0,14);
...
//		}
//	}
//	//	if(USB_USART_RX_STA!=14)
//	//	{
//	//			memset(res,0,14);	
//	//			USB_USART_RX_STA=0;
//	//	}	
//	CRCX=get_crc8_check_sum(&res[1],12,0xff);
//	if(res[13]==CRCX)
//	{
//		memcpy(PcData.PcDataArray,res,MINIPC_FRAME_LENGTH);
//		USB_USART_RX_STA=0;
//		flag=1;
...
/******************************CRC校验*******************************/
		if(res == 0x5A && (USB_USART_RX_STA&0x4000) == 0)			//接收刚开始
		{
			USB_USART_RX_STA = 0;
		...
		if(USB_USART_RX_STA&0x4000)
		{
			...
			{
				USB_USART_RX_BUF[USB_USART_RX_STA&0X3FFF]=res;
				CRCX=get_crc8_check_sum(&USB_USART_RX_BUF[1],12,0xff);
				if(USB_USART_RX_BUF[ MINIPC_FRAME_LENGTH-1 ] != CRCX)
				{
			...
			if((USB_USART_RX_STA&0X3FFF) > MINIPC_FRAME_LENGTH)
			{
