...
void chassis_L_init()//1200 30000   300
{
	...
	leg_left.leg_pid.Kp=900;	  leg_left.leg_pid.Ki=0;    leg_left.leg_pid.Kd=30000;   leg_left.leg_pid.max_iout=0;    leg_left.leg_pid.max_out=300.0f;
	leg_left.turn_pid.Kp=0.5; 	leg_left.turn_pid.Ki=0; 	leg_left.turn_pid.Kd=0.1; 	leg_left.turn_pid.max_iout=0; 	leg_left.turn_pid.max_out=3;
	leg_left.theta_error_pid.Kp=40; 	leg_left.theta_error_pid.Ki=0; 	leg_left.theta_error_pid.Kd=0; 	leg_left.theta_error_pid.max_out=10; 	leg_left.theta_error_pid.max_iout=0;
	leg_left.roll_pid.Kp=5;	  leg_left.roll_pid.Ki=0;    leg_left.roll_pid.Kd=-1;   leg_left.roll_pid.max_iout=0;    leg_left.roll_pid.max_out=40.0f;
...
void chassis_R_init()
{	
	leg_right.right_leg_pid.Kp=900;	  leg_right.right_leg_pid.Ki=0;    leg_right.right_leg_pid.Kd=30000;   leg_right.right_leg_pid.max_iout=0;    leg_right.right_leg_pid.max_out=300.0f;
...
void chassis_L_task()
{
	...
	/*****************left************************/	
		...
		leg_left.v_set=rc_ctrl.rc.ch[1]/660.0f*2.5f;		
		
		leg_left.wheel_T=2.0f*leg_left.LQR_K[0]*(leg_left.theta-0.0f)         //负为正向
										+leg_left.LQR_K[1]*(leg_left.d_theta*0.3f-0.0f)
////										 -leg_left.LQR_K[2]*(leg_left.x_filter-leg_left.x_set)		
										 -2.0f*leg_left.LQR_K[3]*(Kalman0.xhat_data[0]+leg_left.v_set)                  //-
										 +2.0f*leg_left.LQR_K[4]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad
									   +leg_left.LQR_K[5]*(Angular_Handler.V_X-0.0f)*TheattoRad;

				 leg_left.Tp=-5.0f*leg_left.LQR_K[6]*(leg_left.theta-0.0f)      //+
										 -leg_left.LQR_K[7]*(leg_left.d_theta*0.8f-0.0f)        //-
//		
  									 +leg_left.LQR_K[9]*(Kalman0.xhat_data[0]-leg_left.v_set)                //+
										 -10.0f*leg_left.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad          //-
										 -leg_left.LQR_K[11]*(Angular_Handler.V_X*0.2f-0.0f)*TheattoRad;               //-
		leg_left.Tp=leg_left.Tp-leg_left.theta_error_T;
		
	//9025电机直径89mm		
		leg_left.L0_set=0.22f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f;
...
//		leg_left.F0=0;
		...
		if(leg_left.recover_flag==1&&leg_left.FN<20&&fabs(leg_right.L0_set-leg_right.last_L0_set)<0.0001f)
		{
				leg_left.leg_flag=1;
		}
		else
		...
		{
				leg_left.wheel_T=0;
				leg_left.Tp=-5.0f*leg_left.LQR_K[6]*(leg_left.theta-0.0f)-leg_left.LQR_K[7]*(leg_left.d_theta_filter-0.0f);				//-
...
//		leg_right.x_filter=leg_right.x_filter+MF9025[1].RealSpeed*TheattoRad*0.0445f*Chassis_task_Time;
		...
		leg_right.wheel_T=2.0f*leg_right.LQR_K[0]*(leg_right.theta-0.0f)
										  +leg_right.LQR_K[1]*(leg_right.d_theta_filter*0.3f-0.0f)
////											-leg_right.LQR_K[2]*(leg_right.x_filter-leg_right.x_set)
										  +2.0f*leg_right.LQR_K[3]*(Kalman2.xhat_data[0]-leg_left.v_set)                   //+   
										  +2.0f*leg_right.LQR_K[4]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad
										  +leg_right.LQR_K[5]*(Angular_Handler.V_X-0.0f)*TheattoRad;

				leg_right.Tp=-5.0f*leg_right.LQR_K[6]*(leg_right.theta-0.0f)           //-
										 -leg_right.LQR_K[7]*(leg_right.d_theta_filter*0.8f-0.0f)           //-
//		
										 +leg_right.LQR_K[9]*(Kalman2.xhat_data[0]+leg_left.v_set)                //+
										 -10.0f*leg_right.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad               //+
										 -leg_right.LQR_K[11]*(Angular_Handler.V_X*0.2f-0.0f)*TheattoRad;	                   //-
		
		leg_right.Tp=leg_right.Tp+leg_left.theta_error_T;		
		
		leg_right.last_L0_set=leg_right.L0_set;
		leg_right.L0_set=0.22f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f;
...
//		leg_right.F0=0;		
		...
		if(leg_right.recover_flag==1&&leg_right.FN<20&&fabs(leg_right.L0_set-leg_right.last_L0_set)<0.0001f)
		{
		...
		{
				leg_right.wheel_T=0;
				leg_right.Tp=-5.0f*leg_right.LQR_K[6]*(leg_right.theta-0.0f)-leg_right.LQR_K[7]*(leg_right.d_theta_filter*0.5f-0.0f);				//-	
		}
