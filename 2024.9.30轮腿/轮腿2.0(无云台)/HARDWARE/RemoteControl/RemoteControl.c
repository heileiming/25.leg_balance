...
int input_mode=0;
int last_input_mode=0;

//取正函数
static int16_t RC_abs(int16_t value);
//遥控器处理函数
static void SBUS_TO_RC(volatile const uint8_t *sbus_buf, RC_ctrl_t *rc_ctrl);
static void RC_MODE_CONTROL(void);
/*******************************************
遥控器控制变量
********************************************/
...
//    }
    ...
    if (rc_ctrl.rc.ch[0] == -660 && rc_ctrl.rc.ch[1] == -660 && rc_ctrl.rc.ch[2] == 660 && rc_ctrl.rc.ch[3] == -660) //拨杆内八软件强制复位
    {
		...
		if(rc_ctrl.rc.s[switch_right]==switch_down&&rc_ctrl.rc.s[switch_left]==switch_down)
		{
			last_input_mode=input_mode;
			input_mode=stop_mode;
		}
		else if(rc_ctrl.rc.s[switch_left]==switch_down&&rc_ctrl.rc.s[switch_right]==switch_mid)
		{
			last_input_mode=input_mode;			
			input_mode=init_mode;
		}		
		else if(rc_ctrl.rc.s[switch_left]==switch_mid&&rc_ctrl.rc.s[switch_right]==switch_mid)
		{
			last_input_mode=input_mode;			
			input_mode=remote_mode;
		}
		else if(rc_ctrl.rc.s[switch_left]==switch_mid&&rc_ctrl.rc.s[switch_right]==switch_down)
		{
			input_mode=zero_mode;
		}
		else if(rc_ctrl.rc.s[switch_left]==switch_up&&rc_ctrl.rc.s[switch_right]==switch_mid)
		{
			last_input_mode=input_mode;			
			input_mode=spin_mode;			
		}
//		else if((rc_ctrl.rc.s[switch_right]==switch_up&&rc_ctrl.rc.s[switch_left]==switch_mid)||(rc_ctrl.rc.s[switch_right]==switch_up&&rc_ctrl.rc.s[switch_left]==switch_up))
//		{
//			input_mode=pc_mode;
//		}
}
