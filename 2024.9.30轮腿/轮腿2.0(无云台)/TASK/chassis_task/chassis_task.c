...
//	
{  -91.6431 , 128.2059 , -84.1724  , -4.3127},
{  -13.5915 ,  15.7846 , -14.3696  , -1.4091},
{   -0.0051 ,   0.0071 ,  -0.0035  , -0.0025},
{   -7.1631 ,  10.0457 ,  -5.0067  , -3.5593},
{ -102.8154 , 176.1906 ,-118.3971  , 37.5019},
{   -9.4223 ,  15.9754 , -10.6922  ,  4.3493},
{  112.2105 , -95.3361 ,   6.8766  , 18.1282},
{   -7.4947 ,  19.5987 , -18.0303  ,  7.0919},
{   -0.0029 ,   0.0135 ,  -0.0133  ,  0.0045},
{   -4.1953 ,  19.1451 , -18.8215  ,  6.3776},
{  425.6812 ,-542.9867 , 249.6566  , 33.3756},
{   39.7772 , -52.6365 ,  25.6489  ,  2.6145}
...
void chassis_L_init()//1200 30000   300
{
	...
	leg_left.leg_pid.Kp=800;	  leg_left.leg_pid.Ki=0;    leg_left.leg_pid.Kd=12000;   leg_left.leg_pid.max_iout=0;    leg_left.leg_pid.max_out=200.0f;
	...
	leg_left.theta_error_pid.Kp=40; 	leg_left.theta_error_pid.Ki=0; 	leg_left.theta_error_pid.Kd=0.05f; 	leg_left.theta_error_pid.max_out=10; 	leg_left.theta_error_pid.max_iout=0;
	leg_left.roll_pid.Kp=0.1f;	  leg_left.roll_pid.Ki=0.0f;    leg_left.roll_pid.Kd=-0.7f;   leg_left.roll_pid.max_iout=0;    leg_left.roll_pid.max_out=40.0f;//40
...
void chassis_R_init()
{	
	leg_right.right_leg_pid.Kp=800;	  leg_right.right_leg_pid.Ki=0;    leg_right.right_leg_pid.Kd=12000;   leg_right.right_leg_pid.max_iout=0;    leg_right.right_leg_pid.max_out=200.0f;
...
////										 -leg_left.LQR_K[2]*(leg_left.x_filter-leg_left.x_set)		
										 +leg_left.LQR_K[3]*(-Kalman0.xhat_data[0]-leg_left.v_set)*0.7f                  //-
				 ...
				 leg_left.Tp=-leg_left.LQR_K[6]*(leg_left.theta-0.0f)*2.0f      //+
										 -leg_left.LQR_K[7]*(leg_left.d_theta_filter*leg_theta_K-0.0f)        //-
//		
  									 -leg_left.LQR_K[9]*(-Kalman0.xhat_data[0]-leg_left.v_set)*0.8f               //+
										 -leg_left.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad*2.0f          //-
...
//				
//		}
		leg_left.L0_set=0.25f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f-0.40f*arm_sin_f32((Angular_Handler.Pitch-pitch_offect)*TheattoRad);	
...
//		leg_left.F0=0;
		...
		{			leg_left.v_set=0.4f;
			leg_right.v_set=-0.4f;

			leg_left.last_yaw=Angular_Handler.YAW;
			leg_left.spin_flag=0;
			leg_left.turn_T=0;
		}
		else
		{
...
/****************离地检测*****************/			
		leg_left.un_F0=(leg_left.j22*DM_data[0].torque-leg_left.j12*DM_data[1].torque)/(leg_left.j11*leg_left.j22-leg_left.j12*leg_left.j21);
		leg_left.un_Tp=(-leg_left.j21*DM_data[0].torque+leg_left.j11*DM_data[1].torque)/(leg_left.j11*leg_left.j22-leg_left.j12*leg_left.j21);
		
		leg_left.FN=leg_left.un_F0*arm_cos_f32(leg_left.theta)-leg_left.un_Tp*arm_sin_f32(leg_left.theta)/leg_left.L0+wheel_mg;   //支持力		
		if(leg_left.recover_flag==1&&leg_left.FN<20)
		{
				leg_left.leg_flag=1;
		}
		else
		...
		{
				leg_left.wheel_T=0;
				leg_left.Tp=-2.0f*leg_left.LQR_K[6]*(leg_left.theta-0.0f)-leg_left.LQR_K[7]*(leg_left.d_theta_filter*leg_theta_K-0.0f);				//-
		}		
/****************离地检测*****************/			
		...
		if(input_mode==remote_mode||input_mode==spin_mode)
		{
			...
			leg_left.yaw_set+=rc_ctrl.rc.ch[0]/660.0f/10.0f*4.0f;
...
////											-leg_right.LQR_K[2]*(leg_right.x_filter-leg_right.x_set)
										  +leg_right.LQR_K[3]*(Kalman2.xhat_data[0]-leg_right.v_set)*0.7f                    //+   
				...
				leg_right.Tp=-leg_right.LQR_K[6]*(leg_right.theta-0.0f)*2.0f         			 //-
										 -leg_right.LQR_K[7]*(leg_right.d_theta_filter*leg_theta_K-0.0f)           //-
//		
										 -leg_right.LQR_K[9]*(Kalman2.xhat_data[0]-leg_right.v_set)*0.8f                //+
										 -leg_right.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad*2.0f             //+
...
//					
//		}
		leg_right.L0_set=0.25f+(rc_ctrl.rc.ch[2]/660.0f)/10.0f*2.0f+0.40f*arm_sin_f32((Angular_Handler.Pitch-pitch_offect)*TheattoRad);
...
/****************离地检测*****************/		
		leg_right.un_F0=(leg_right.j22*DM_data[3].torque-leg_right.j12*DM_data[2].torque)/(leg_right.j11*leg_right.j22-leg_right.j12*leg_right.j21);
		leg_right.un_Tp=(-leg_right.j21*DM_data[3].torque+leg_right.j11*DM_data[2].torque)/(leg_right.j11*leg_right.j22-leg_right.j12*leg_right.j21);
		...
		if(leg_right.recover_flag==1&&leg_right.FN<20)
		{
		...
		{
				leg_right.wheel_T=0;
				leg_right.Tp=-2.0f*leg_right.LQR_K[6]*(leg_right.theta-0.0f)-leg_right.LQR_K[7]*(leg_right.d_theta_filter*leg_theta_K-0.0f);				//-	
		}	
/****************离地检测*****************/			
