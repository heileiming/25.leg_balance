...
void chassis_L_init()//1200 30000   300
{
	...
	leg_left.leg_pid.Kp=900;	  leg_left.leg_pid.Ki=0;    leg_left.leg_pid.Kd=20000;   leg_left.leg_pid.max_iout=0;    leg_left.leg_pid.max_out=200.0f;
	...
	leg_left.roll_pid.Kp=6;	  leg_left.roll_pid.Ki=0;    leg_left.roll_pid.Kd=-0.5f;   leg_left.roll_pid.max_iout=0;    leg_left.roll_pid.max_out=40.0f;//40
...
void chassis_R_init()
{	
	leg_right.right_leg_pid.Kp=900;	  leg_right.right_leg_pid.Ki=0;    leg_right.right_leg_pid.Kd=20000;   leg_right.right_leg_pid.max_iout=0;    leg_right.right_leg_pid.max_out=200.0f;
...
void chassis_L_task()
{
	...
	/*****************left************************/	
		...
		leg_left.wheel_T=leg_left.LQR_K[0]*(leg_left.theta-0.0f)*2.0f         //负为正向
										+leg_left.LQR_K[1]*(leg_left.d_theta_filter*wheel_theta_K-0.0f)
////										 -leg_left.LQR_K[2]*(leg_left.x_filter-leg_left.x_set)		
										 +leg_left.LQR_K[3]*(-Kalman0.xhat_data[0]-leg_left.v_set)*0.75f                  //-
				 ...
				 leg_left.Tp=-leg_left.LQR_K[6]*(leg_left.theta-0.0f)*3.0f      //+
										 -leg_left.LQR_K[7]*(leg_left.d_theta_filter*leg_theta_K-0.0f)        //-
//		
  									 -leg_left.LQR_K[9]*(-Kalman0.xhat_data[0]-leg_left.v_set)                //+
										 -leg_left.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad*10.0f          //-
...
//		leg_left.F0=0;
		...
		if(leg_left.recover_flag==1&&leg_left.FN<40)
		{
				leg_left.leg_flag=1;
		}
		else
...
//		leg_right.x_filter=leg_right.x_filter+MF9025[1].RealSpeed*TheattoRad*0.0445f*Chassis_task_Time;
		...
		leg_right.wheel_T=leg_right.LQR_K[0]*(leg_right.theta-0.0f)*2.0f
										  +leg_right.LQR_K[1]*(leg_right.d_theta_filter*wheel_theta_K-0.0f)
////											-leg_right.LQR_K[2]*(leg_right.x_filter-leg_right.x_set)
										  +leg_right.LQR_K[3]*(Kalman2.xhat_data[0]-leg_left.v_set)*0.75f                    //+   
				...
				leg_right.Tp=-leg_right.LQR_K[6]*(leg_right.theta-0.0f)*3.0f           //-
										 -leg_right.LQR_K[7]*(leg_right.d_theta_filter*leg_theta_K-0.0f)           //-
//		
										 -leg_right.LQR_K[9]*(Kalman2.xhat_data[0]-leg_left.v_set)                //+
										 -leg_right.LQR_K[10]*(Angular_Handler.ROLL-MPU_offect)*TheattoRad*10.0f              //+
...
//		leg_right.F0=0;		
		...
		if(leg_right.recover_flag==1&&leg_right.FN<40)
		{
		...
		leg_right.wheel_T=-fp_Limit_mouse(leg_right.wheel_T,-4.0f,4.5f);
		if(input_mode==remote_mode)
		{		
